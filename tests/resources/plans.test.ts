...
describe('PlansResource', () => {
  ...
  it('list() returns plans map', async () => {
    const plans = { '1': { pid: '1', plan_name: 'basic', disk: '20480', ram: '1024', bandwidth: '1000', cpu: '1' } };
    const client = makeClient({ plans });
  ...
  it('create() calls act=addplan with params as body', async () => {
    ...
    await new PlansResource(client).create({ plan_name: 'starter', disk: 20480, ram: 1024, bandwidth: 1000, cpu: 1 });
    expect(client.request).toHaveBeenCalledWith('addplan', {}, expect.objectContaining({ plan_name: 'starter' }));
  });
